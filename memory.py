...
async def process_conversation_memory(
    ...
    # 4. totalMemories作成処理
    await _generate_total_memories(
        client_id, session_store, session_store_lock, chat_req, a_client
    )
    return
...
async def _generate_total_memories(
    client_id: str,
    session_store: Dict[str, Any],
    session_store_lock,
    chat_req,
    ...
    """
    shortMemories、midMemories、longMemoriesから総合的な要約を作成してtotalMemoriesに保存する
    ...
    try:
        ...
        with session_store_lock:
            if client_id not in session_store:
                print(f"_generate_total_memories: client_id {client_id} がセッションストアに存在しません")
                return
                
            memory_store = session_store[client_id].get("memory_store", {})
            short_memories = memory_store.get("shortMemories", "")
            mid_memories = memory_store.get("midMemories", "")
            long_memories = memory_store.get("longMemories", "")
        
        # 各メモリが存在するかチェック
        memories_to_combine = []
        if short_memories.strip():
            memories_to_combine.append(f"【直近の記憶】\n{short_memories}")
        if mid_memories.strip():
            memories_to_combine.append(f"【中期の記憶】\n{mid_memories}")
        if long_memories.strip():
            memories_to_combine.append(f"【長期の記憶】\n{long_memories}")
        
        # 結合可能なメモリが存在する場合のみ総合要約を作成
        if memories_to_combine:
            combined_memories = "\n\n".join(memories_to_combine)
            
            # AI による総合要約の生成
            user_msg = (
                "以下のテキストは、ユーザーとアシスタントの会話における短期・中期・長期の記憶をまとめたものです。"
                "これらの内容を統合し、600文字程度でユーザーの特徴、好み、継続的な関心事、重要な出来事などを含む"
                "包括的な要約を作成してください。時系列を意識し、重要な固有名詞や継続性のあるテーマを重視してください。"
                "作成の指針は以下の通り:\n"
                "1. 長期の記憶は比較的曖昧だけど、網羅的に要約する\n"
                "2. 中期の記憶は具体的な出来事やテーマを中心に要約する\n"
                "3. 短期の記憶は最新の出来事やユーザーの関心を反映する\n"    
                "絵文字は無視してください。\n\n統合するメモリ内容:\n" + combined_memories
            )
            ...
            response_log = await chat_req(a_client, user_msg, "あなたは賢いAIです。userのリクエストに必ず日本語で答えること。リクエストの回答以外は答えないこと。")
            response_log = f"[{now}] {response_log}"
            print("総合要約をtotalMemoriesに保存:", response_log)
            
            # totalMemoriesに保存
            with session_store_lock:
                if client_id in session_store:
                    session_store[client_id]["memory_store"]["totalMemories"] = response_log
                    print(f"デバッグ: totalMemoriesを更新しました - client_id: {client_id}")
        else:
            print("デバッグ: 総合要約に使用できるメモリが存在しないため、totalMemories作成をスキップします")
            
    except Exception as e:
        print(f"totalMemories生成中にエラーが発生しました: {e}")
